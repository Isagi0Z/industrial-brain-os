...
class PostgresDocumentRepository(IDocumentRepository):
    ...
    def soft_delete(self, document_id: str) -> None:
        ...
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE documents SET is_deleted = TRUE WHERE id = %s", (document_id,)
            )
            conn.commit()
    ...
    def restore(self, document_id: str) -> None:
        ...
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE documents SET is_deleted = FALSE WHERE id = %s", (document_id,)
            )
            conn.commit()
    ...
    def get_metadata(self, document_id: str) -> Optional[DocumentMetadata]:
        ...
        with conn.cursor() as cur:
            cur.execute(
                "SELECT id, document_id, metadata, created_at, updated_at FROM document_metadata WHERE document_id = %s",
                (document_id,),
            )
            row = cur.fetchone()
            if not row:
