...
class DocumentUseCase:
    def __init__(
        self, document_repo: IDocumentRepository, storage_service: IStorageService
    ):
    ...
    def upload_document(
        ...
        if size_bytes > MAX_FILE_SIZE_BYTES:
            raise DomainException(
                "FILE_TOO_LARGE",
                f"File size exceeds limit of {MAX_FILE_SIZE_BYTES} bytes",
            )

        # Validate MIME
        if mime_type not in ALLOWED_MIME_TYPES:
            raise DomainException(
                "INVALID_MIME_TYPE", f"MIME type {mime_type} is not allowed"
            )
        ...
        if existing_doc:
            raise DomainException(
                "DUPLICATE_FILE", "A document with the same content already exists"
            )
    ...
    def update_metadata(
        ...
        self.get_document(document_id)  # validates existence; raises if not found
        now = datetime.now(timezone.utc)
    ...
    def get_presigned_download_url(
        ...
        if version_number is not None:
            for v in versions:
                ...
                raise DomainException(
                    "VERSION_NOT_FOUND", f"Version {version_number} not found"
                )

        return self.storage_service.get_presigned_download_url(
            self.bucket_name, target_version.storage_path
        )
